...
class AudioController:
    ...
    def set_default_volumes(self):
        try:
            ...
            print(f"[Fehler] Default-Lautstärken konnten nicht gesetzt werden: {e}")
    ...
    def set_master_volume(self, volume):
        ...
        try:
            # Setze Lautstärke für den Master-Sink
            subprocess.run(["amixer", "set", "Master", f"{volume}%"], check=True)
            print(f"🔊 Lautstärke für Master auf {volume}% gesetzt.")
        except Exception as e:
            print(f"[Fehler] Lautstärke für Master konnte nicht gesetzt werden: {e}")